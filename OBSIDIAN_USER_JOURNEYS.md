...
|---|---|---|---|
| P0-1 | ✅ **RESOLVED 2026-06-09.** The slash commands were missing because the Phase 3 port copied only `commands/qmd/` and left the others behind. Ported the 17 missing files and organized all four skills' commands into per-skill folders: `secondbrain/` (10), `rituals/` (3), `manage/` (4), `qmd/` (4). Verified against the official Claude Code docs that **subdirectories are purely organizational and do NOT namespace** — every command invokes as its bare name (`/open-day`, `/week-prep`, `/task-sync`, `/ask`) regardless of folder; no basename collisions. (An earlier note here wrongly claimed `/rituals:week-prep` namespacing — that was incorrect.) **Remaining:** no Obsidian-side trigger yet (see P2-1). | fixed in commits `4d7a4e7`, then reorganized | done |
