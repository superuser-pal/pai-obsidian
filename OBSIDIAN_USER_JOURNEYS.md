...
|---|---|---|---|
| P0-1 | ✅ **RESOLVED 2026-06-09.** The slash commands were missing because the Phase 3 port copied only `commands/qmd/` and left the others behind. Ported the 17 missing files: SecondBrain (10, root), DailyRituals (`rituals/`, 3), ProjectManagement (`manage/`, 4). **Remaining:** the `rituals/`/`manage/` subdir namespacing means invocation is `/rituals:week-prep`, not the bare `/week-prep` the SKILL.md tables document (minor doc/flatten follow-up); and there's still no Obsidian-side trigger (see P2-1). | was: `commands/` lacked them; fixed in commit `4d7a4e7` | done |
